...
<head>
	...
	<script src="<%=request.getContextPath()%>/js/contractIndexMode.js"></script>
...
<body>
	...
	<div class="mRight">
		<div class="mRpos">
			<ol class="breadcrumb">
				...
				<li><a href="#">合同管理</a></li>
				<li><a href="#">合同台帐</a></li>
				<li class="active">修改台帐</li>
			</ol>
		</div>
		<div class="varyBox">
			<div class="btnlist">
				<a href="#" class="btn_blue" id="sure" onclick="addContractList();" >确认</a>
				<a href="#" id="close" class="btn_red"  onclick="history.go(-1);" >返回</a>
			</div>
			<p class="varyTitBg">修改台帐</p>
			<div class="tabBoxBase varyInnerBox">
				<p class="varyTitSm">合同概述</p>
				<div class="varyInput clauseHeight">
				<form id="form1">
				<input type="hidden" id="contractId" name="contractId" value="${contractInfo.id }">
				<input type="hidden" id="signTime1" value="${contractInfo.signTime }">
				<input type="hidden" id="contractStartTime1" value="${contractInfo.contractStartTime }">
				<input type="hidden" id="contractEndTime1" name="contractEndTime1" value="${contractInfo.contractEndTime }">
					<table class="table table-bordered ">
						...
						<tbody>
							<tr>
								...
								<td colspan="3"><input type="text" class="input" id="contractName" name="contractName" value="${contractInfo.contractTitle }"/></td>
							</tr>
							<tr>
								<td class="varyTabTit">甲方单位*</td>
								<td><input type="text" class="input" id="firstPartyUnit" name="firstPartyUnit" value="${contractInfo.firstPartyUnit }"/></td>
								<td class="varyTabTit">乙方单位*</td>
								<td><input type="text" class="input" id="secondPartyUnit" name="secondPartyUnit" value="${contractInfo.secondPartyUnit }"/></td>
							</tr>
							<tr>
								<td class="varyTabTit">甲方负责人*</td>
								<td><input type="text" class="input" id="firstPartyPrincipal" name="firstPartyPrincipal" value="${contractInfo.firstPartyPrincipal }"/></td>
								<td class="varyTabTit">乙方负责人*</td>
								<td><input type="text" class="input" id="secondPartyPrincipal" name="secondPartyPrincipal" value="${contractInfo.secondPartyPrincipal }"/></td>
							</tr>
							<tr>
								<td class="varyTabTit">甲方联系方式*</td>
								<td><input type="text" class="input" id="firstPartyContactWay" name="firstPartyContactWay" value="${contractInfo.firstPartyContactWay }"/></td>
								<td class="varyTabTit">乙方联系方式*</td>
								<td><input type="text" class="input" id="secondPartyContactWay" name="secondPartyContactWay" value="${contractInfo.secondPartyContactWay }"/></td>
							</tr>
							<tr>
								...
								<td><input type="text" class="input" id="signTime" name="signTime"/></td>
							</tr>
								<tr>
								<td class="varyTabTit">合同金额*</td>
								<td>
									<input type="text" class="input" id="contractMoney" name="contractMoney" value="${contractInfo.contractMoney }"/>									
								</td>
							...
							<tr>
								...
								<td><input type="text" id="contractStartTime" name="contractStartTime" class="input"/></td>
								<td class="varyTabTit">合同结束时间*</td>
								<td><input type="text" class="input" id="contractEndTime" name="contractEndTime"/></td>
							</tr>
							<tr>
								<td style="height:80px; line-height:80px;" class="varyTabTit">合同标的</td>
								<td colspan="3"><textarea class="conTextIn" id="contractObject">${contractInfo.contractObject }</textarea></td>								
							</tr>
							<tr>
								...
								<td colspan="3">
									...
									<c:forEach items="${selectPlanFile }" var="file">	
										<div title="${file.sfName }" class='drFileItem' id="${file.sfId }"><i class='fa fa-file'></i><br><a class='drFileName drFileNames' href='javascript:;'>${file.sfName }</a><a class='drDel btn_green'  href='#' onclick="downFile(${file.sfId },${file.contractFileId })">下载</a><a class='drDel btn_red proEditBtn' href='javascript:void(0);' onclick="delFile(${file.sfId },${file.contractFileId })">删除</a>
										</div>
									</c:forEach>
									</div>
							...
							<tr>
								...
								<td colspan="3"><textarea class="conTextIn" id="contractDesc">${contractInfo.contractDesc }</textarea></td>								
							</tr>
						</tbody>
					</table>
					</form>
			...
			<div class="tabBoxFit varyInnerBox">
				...
				<div class="varyInput clauseHeight" id="payTerm">
					<div data-mes='0' class="termList" style="display:none;">
					</div>
				    <c:forEach items="${contractPaymentTerms }" var="terms">
				    <div data-mes="" class='termList interClose' data-min="${terms.id }" ><i class='closeBtn'>×</i>
					<p class='termTit'>条件</p>
					
					<div class='termBox'>
				    <div class='termBoxList'>
					<span class='span1'>条件：</span>						
					<input type='text'  value='${terms.name }' class='input' style='width:240px'>	</div>
					<div class='termBoxList'>
					<input name='payFor"+dataMes+"' value="1" type='radio' class='radio' checked/><span class='span2'>付款金额：</span>
					<input type='text'  value='${terms.paymentAmount }' class='input'><span>(单位：元)</span></div>
					<div class='termBoxList'>
					<input name='payFor"+dataMes+"' value="2" type='radio'  class='radio'/>
					<span class='span2'>付款百分比：</span>
					<input type='text'  value='${terms.paymentProportion }' class='input'>
	...
	<script type="text/javascript">
	   $(function (){
		   var signTime1 = FormatDate($("#signTime1").val());
		   var contractStartTime1 = FormatDate($("#contractStartTime1").val());
		   var contractEndTime1 = FormatDate($("#contractEndTime1").val());
		   $("#signTime").val(signTime1);
		   $("#contractStartTime").val(contractStartTime1);
		   $("#contractEndTime").val(contractEndTime1);
	   })
...
$("#addTerm").click(function(){
	...
	var str="<div data-mes='"+(dataMes+1)+"' class='termList'><i class='closeBtn'>×</i>"+
			    ...
			    "<div class='termBoxList'>"+
				"<span class='span1'>条件：</span>"+							
				"<input type='text'  value='' class='input' style='width:240px'>	</div>"+
				"<div class='termBoxList'>"	+
				"<input name='payFor"+dataMes+"' value='1' type='radio' class='radio' checked/><span class='span2'>付款金额：</span>"+
				"<input type='text'  value='' class='input'><span>(单位：元)</span></div>"+
				"<div class='termBoxList'>"+
				"<input name='payFor"+dataMes+"' value='2' type='radio'  class='radio'/>"+
				"<span class='span2'>付款百分比：</span>"+
...
$(".interClose .closeBtn").click(function(){
	var str=$(this).parent().attr("data-min");
	document.getElementById("payId").value+=str+",";
	$(this).parent().remove();
	var nbr=0;
	$.each($(".termList"),function(){
		if($(this).find("input:radio:checked").val()=="1"){
