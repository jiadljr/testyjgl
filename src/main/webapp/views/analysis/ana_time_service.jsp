...
<title>Insert title here</title>
	<link rel="stylesheet" type="text/css" href="<%=request.getContextPath() %>/plugins/jQuery_tree/all.css"><script src="<%=request.getContextPath()%>/js/jquery.js"></script>
        	...
        	<div class="accordion" fillSpace="sidebar">
           		<div class="accordionContent">
              		<ul id="a" class="tree treeFolder collapse">
                		<li class="tree_list" style="display: block;"><a autobypy="bz" dataType="topmenu" childMneu="bzmenus" keyboard="A" style="color:#183152" id="openTree">事件源</a>
...
<script type="text/javascript">
$(function(){
	var url = getPath()+"/selectServiceTypeAll.do";
	var serviceOne;
	var serviceTwo;
	var serviceThree;
	$.ajax({
		...
		success:function(result){
			...
			$.each(serviceOne,function(i,one){
				var serviceType = "<li  class=\"deptOne\"><a autobypy="+one.extend2 +"href=\"#\" onclick=\"queryServiceType("+serviceOne[i].id+",\'"+one.name+"\')\">";
				serviceType += "<span class=\"font_pos\"><i class=\"external_small\"></i></span>"+one.name+"</a><ul id=\"one"+i+"\">";
				$.each(serviceTwo,function(j,two){
					if(serviceOne[i].id == serviceTwo[j].parentId){
						serviceType += "<li class=\"deptTwo\"><a autobypy="+two.extend2 +"href=\"#\" onclick=\"queryServiceType("+ serviceTwo[j].id + ",\'"+two.name+"\')\">";
						serviceType += "<span class=\"font_pos\"><i class=\"external_small\"></i></span>"+two.name+"</a><ul id=\"two"+j+"\">";	
						$.each(serviceThree,function(k,three){
							if (serviceTwo[j].id == serviceThree[k].parentId) {
								serviceType += "<li  class=\"deptThree\"><a autobypy="+three.extend2 +"href=\"#\" onclick=\"queryServiceType("+serviceThree[k].id+",\'"+three.name+"\')\">";
				...
				serviceType += "</ul></li>"; 
				$(".bzmenus_sub").append(serviceType);
				$("a").click(function(){
					$("a").css({"color":"#183152"});
					$(this).css({"color":"red"});
	...
	function queryServiceType(id,name){
		var url = getPath()+"/selectServiceTypePareateId.do";
		$.ajax({
			url:url,
			data:{'id':id},
			dataType:'json',
	...
	function sureAssets() {
		...
		$('#serviceId',parent.document).val(arrayId);
		$('#serviceName',parent.document).val(arrayName);
		//alert(array);
