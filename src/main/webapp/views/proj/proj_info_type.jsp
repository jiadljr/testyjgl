...
<script src="<%=request.getContextPath()%>/js/proj_config_type.js"></script>
<style>
...
<body>
			<input type="hidden" id="typeId" value="${projTypeOne.id }"/>
 			<input type="hidden" id="updtypeName" value="${projTypeOne.name }"/>
 			<input type="hidden" id="updtypeRemark" value="${projTypeOne.remark }"/>
 			<input type="hidden" id="updtypeColor" value="${projTypeOne.color }"/>
	...
	<form id="form1">
 	...
 	<div class="setType">
 		...
 		<ul class="setTypeList">
 			<c:forEach items="${projTypeList }" var="projType">
 				<c:if test="${projType.name != null && projType.name != ''}">
	 				<li><a href="#" onclick="showTypeInform(${projType.id })" id="typeId${projType.id }" >${projType.name }</a></li>
 				</c:if>
 			</c:forEach>
 		</ul>
 		<div class="addtype">
 			...
 			<div class="pList" style="margin-top:30px;">
 				<label>类型名称：</label>
 				<input type="text" id="typeName" name="typeName" disabled="disabled" class="template needIn"  style="background:#fff;border:none;" value="${projTypeOne.name }">
 			</div>
			<div class="pList">
 				<label>颜色背景：</label>
 				<input type="color" style="width:70px; border:none;" value="${projTypeOne.color }"  disabled="disabled"  id="typeColor" name="typeColor" >
 			</div>
 			<div class="pList" style="height:80px;">
 				<label style="line-height:28px;">类型说明：</label>
 				<textarea id="typeRemark" name="typeRemark" class="template needIn" disabled="disabled" style="background:#fff;border:none;">${projTypeOne.remark }</textarea>
 			</div>
...
<script type="text/javascript">
	$(function(){
		//获取第一个setTypeList下的第一个li加hover属性
		$(".setTypeList li a").eq(0).addClass("hover")
