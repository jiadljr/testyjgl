...
<body>
			...
			<div class="taskBox">
				<form role="form" class="form-horizontal" id="form1" method="post">
					...
					<input type="hidden" name="vs" value="${task.vs }"/>
					<div class="taskList">
						<label for="taskName">任务名称<i style="color:red;font-size:16px;">*</i>：</label>
						<input type="text" id="taskName" name="taskName" class="needIn" value="${task.nameTask }" data-message="项目名称不得为空！">
					</div>
					<div class="taskList">
						...
						<select id="parentId" name="parentId" class="">
							...
							<c:forEach items="${projTaskList }" var="projTask">
								<option value = "${projTask.id }" <c:if test="${task.parentId == projTask.id}">selected</c:if>>${projTask.nameTask }</option>
							</c:forEach>
						</select>
					</div>
					<div class="taskList">
					...
					<div class="taskList taskTime">
						...
						<input type="text" id="startTime" name="startTime" class="needIn" value="<fmt:formatDate value="${task.dateStart}" pattern="yyyy-MM-dd" />">
						<span>——</span>
						<input type="text" id="endTime" name="endTime" class="needIn" value="<fmt:formatDate value="${task.dateEnd}" pattern="yyyy-MM-dd" />">
					</div>
					<div class="taskList">
						<label for="taskStartDate" >里程碑任务：</label>
						<input type="checkbox" id="_milestoneTask" name="_milestoneTask" class="sDate" <c:if test="${task.milestoneTask == 1 }">checked</c:if>>		
					</div>
					<div class="taskTxt">
						<label for="taskDesc">任务说明：</label>
						
						<textarea id="taskDesc" name="taskDesc" class="form-control" data-message="任务说明不得为空！">${task.taskDesc }</textarea>
						
					</div>
					<div class="taskTxt">
						<label for="taskDesc">阶段成果/文档：</label>
						
						<textarea id="phaseShow" name="phaseShow" class="form-control">${task.phaseShow }</textarea>
		...
		$(function(){
			...
			$("#saveTask").click(function(){
				...
				$.ajax({
					...
					data:{"taskName":taskName,"parentId":parentId,"projCode":projCode,"taskId":taskId},
					dataType:'text',
					type:'post',
