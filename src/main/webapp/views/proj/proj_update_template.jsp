...
<div class="addTemp">
	<div class="tempTit">
		<label>模版名称：</label>
		<input value="${projTemplate.templateName }" type="text" id="templateName"/>
		<input value="${templateId }" type="hidden" id="templateId"/>
	</div>
	<div class="tempItem">
		...
		<ul class="tempInput">
			<c:forEach items="${templateTaskList }" var="templateTask">
				<li><input value="${templateTask.taskName }" type="text" /><span style="display: none">${templateTask.id }</span></li>
			</c:forEach>
		</ul>
	</div>
	<div class="tempBtn">
...
<script type="text/javascript">
	$(function() {
		...
		$("#addTemp").click(function(){
			$(".tempInput").append("<li><input type='text' value=''/><span style='display: none'>0</span></li>");
		...
		$("#arrowUp").click(function(){
			...
			if($prev.prev().length==0){
				...
				var id=$prev.prev().children().next().text();
				var skr=$prev.prev().children().val();
				$prev.prev().remove();
				$prev.after("<li><input type='text' value='"+skr+"' /><span style='display: none'>"+id+"</span></li>")
			}
			$(".tempInput li").click(function(){
				$(".tempInput li").removeClass("hover");
				$(this).addClass("hover")	
		...
		$("#arrowDown").click(function(){
			...
			if($next.next().length==0){
				...
				var id=$next.next().children().next().text();
				var str=$next.next().children().val();
				$next.next().remove();
				$next.before("<li><input value='"+str+"' type='text' /><span style='display: none'>"+id+"</span></li>")				
			}	
			$(".tempInput li").click(function(){
				$(".tempInput li").removeClass("hover");
				$(this).addClass("hover")	
		...
		$("#tempSure").click(function(){
			...
			$.each($(".tempInput li span"),function(){
				idStr.push($(this).text())
			})
			...
			$.ajax({
				url:getPath()+"/updateTemplate.do",
				type:"post",
				dataType:"text",
				data:{"templateId":templateId,"idStr":idStr.toString(),"taskStr":arr.toString(),"numStr":num.toString(),"templateName":templateName},
