...
<body>
		<div class="mRight">
			...
			<div class="mRbot mNewRbot">
				<div class="mRwell">
					<div class="mRwell-tit">修改资产信息</div>
					<form role="form" class="form-horizontal" id="form1" method="post">
						...
						<input type="hidden" name="id" id="assId" value="${seleAss.id }">
						...
						<div class="mRwell-container">
							<div class="mRwell-con-center">
								<div class="form-group">
									...
									<div class="col-sm-9"><input type="text" id="code" name="code" value="${seleAss.code }" class="form-control" data-message="资产编号不得为空！"></div>
								</div>
								<div class="form-group">
									<label for="name" class="col-sm-3">资产名称<span style="color:red;font-size:16px;">*</span>：</label>
									<div class="col-sm-9"><input type="text" id="name" name="name" value="${seleAss.name }" class="form-control" data-message="资产名称不得为空！"></div>
								</div>
								<div class="form-group">
									<label for="sub_name" class="col-sm-3">资产型号<span style="color:red;font-size:16px;">*</span>：</label>
									<div class="col-sm-9"><input type="text" id="asModel" name="asModel" value="${seleAss.asModel }" class="form-control" data-message="资产型号不得为空！"></div>
								</div>						
								<div class="form-group">
									...
									<div class="QK_search">
								<div id="searchMenu">
									<input type="hidden" name="sourceId" id="sourceId" value="${seleAss.idType }" /> 
									<input id="sourceSearchTree" type="text" class="span2 form-control" placeholder="请选择事件源" data-autofocus="autofocus" x-webkit-speech="" lang="zh-CN" x-webkit-grammar="builtin:translate" onwebkitspeechchange="$('#searchTreeBtn').trigger('click')" onclick="showSourceTree()" value="${seleAss.extend1 }">
								</div> 
								...
								<div class="form-group">
									...
									<div class="col-sm-9"><input type="text" id="asManuf" name="asManuf" value="${seleAss.asManuf }" class="form-control" data-message="资产厂家不得为空！" ></div>
								</div>
								<div class="form-group">
							...
							<div class="QK_search">
							<div id="searchMenu">
								<input type="hidden" name="idDept" id="idDept" value="${deptId }" /> 
							    <input id="searchTree" type="text" value="${deptName }" class="span2 form-control" data-autofocus="autofocus" placeholder="请选择部门" x-webkit-speech="" lang="zh-CN" x-webkit-grammar="builtin:translate" onwebkitspeechchange="$('#searchTreeBtn').trigger('click')" onclick="showTree()">
							</div> 
								...
								<div class="form-group">
									...
									<div class="col-sm-9">
										<select id="status" name="status" class="form-control">
											<option value="0" <c:if test="${seleAss.idDept eq '0'}"> selected="selected"</c:if>>正常</option>
											<option value="1" <c:if test="${seleAss.idDept eq '0'}"> selected="selected"</c:if>>故障</option>
										</select>
									</div>
								</div>
								<div class="btnlist">
									<a class="btn btn-primary btn-sm" id="modAssetsOk">确定</a>
