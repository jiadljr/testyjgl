						...
						if($tli.children("a").text().indexOf(filterVal)==-1&&$tli.children("a").attr("autobypy").indexOf(filterVal)==-1){
							...
							if(op.Highlight){
								if($tli.children("a").text().indexOf(filterVal)!=-1){
									...
									/**三级*/
									if ($tli.children("ul").length>0) {
										var $sli=$tli.children("ul").children("li");
										$sli.each(function(){
											var $stli=$(this);
											if($stli.children("a").text().indexOf(filterVal)==-1&&$stli.children("a").attr("autobypy").indexOf(filterVal)==-1){
												if($stli.children("ul").length>0){
													var tul=$stli.children("ul");
													addFilterHtml(tul);
												}
												else{$stli.remove();}
											}else{
												if(op.Highlight){
													if($stli.children("a").text().indexOf(filterVal)!=-1){
														$stli.children("a").html($stli.children("a").text().replace(filterVal,"<b class='color-red'>"+filterVal+"</b>"));
													}else{
														var temptext=$stli.children("a").text().slice($stli.children("a").attr("autobypy").indexOf(filterVal),$stli.children("a").attr("autobypy").indexOf(filterVal)+filterVal.length);
														$stli.children("a").html($stli.children("a").text().replace(temptext,"<b class='color-red'>"+temptext+"</b>"))
													}
													/**四级*/
													if ($stli.children("ul").length>0) {
														var $fsli=$stli.children("ul").children("li");
														$fsli.each(function(){
															var $fstli=$(this);
															if($fstli.children("a").text().indexOf(filterVal)==-1&&$fstli.children("a").attr("autobypy").indexOf(filterVal)==-1){
																if($fstli.children("ul").length>0){
																	var tul=$fstli.children("ul");
																	addFilterHtml(tul);
																}
																else{$fstli.remove();}
															}else{
																if(op.Highlight){
																	if($fstli.children("a").text().indexOf(filterVal)!=-1){
																		$fstli.children("a").html($fstli.children("a").text().replace(filterVal,"<b class='color-red'>"+filterVal+"</b>"));
																	}else{
																		var temptext=$fstli.children("a").text().slice($fstli.children("a").attr("autobypy").indexOf(filterVal),$fstli.children("a").attr("autobypy").indexOf(filterVal)+filterVal.length);
																		$fstli.children("a").html($fstli.children("a").text().replace(temptext,"<b class='color-red'>"+temptext+"</b>"))
																	}
																}
															}
														});
														if($stli.children("ul").children("li").length==0){
															$stli.children("ul").remove();
														};
										...
										if($tli.children("ul").children("li").length==0){
											$tli.children("ul").remove();
										};
